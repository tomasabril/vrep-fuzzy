...
/* definevars  - Três métodos para definir as variáveis linguísticas de entrada
                 e saída que controlam o comportamento do sistema */
...
#ifdef DEBUG
     std::cout << "Activ of Rule " << i << "= ";
     std::cout << activationofrule[i] << "\n";
#endif
   }
   ...
   for(int i=0;i<numrules;i++)
	...
	countrulesfiring++;
  	outcat = rulebase[i]->getoutputcategory();
        outcat->setoutput(x);
#ifdef DEBUG
        std::cout << "Rule: " << i << "\tActivation: " << x << "\t" << outcat->getname()<< "\t" <<outcat->getoutput()<<"\n";
#endif
...
#ifdef DEBUG
   std::cout << "Output: " << outcat->getoutput() << "\tMidval: " <<  outcat->getmidval();
   std::cout << "\tCategory: " << outcat->getname() << "\tDefuzzMOM: " << defuzzout << "\t Sumativ: " << sumativations << "\n";
#endif
...
void fuzzy_control::save_m(char *name, float error)
{
  ...
  if ( (fileFC = fopen ( fileName, "w" )) == NULL )
    {
      std::cout << "Error in file creation:" << fileName << "\n";
      exit(1);
    }
...
void fuzzy_control::load(char *name)
{
  ...
  if ( (fileFC = fopen ( fileName, "r" )) == NULL )
    {
      std::cout << "Error in file opening:" << fileName << "\n";
      exit(1);
    }
